...
export default function TechnicianDashboard() {
  ...
  const overdueTickets = useMemo(
    () =>
      tickets.filter((t) => {
        ...
        const s = (t.status || "").toUpperCase().replace(/_/g, " ");
        return (
          p === "CRITICAL" && !s.includes("RESOLVE") && !s.includes("CLOSE")
        );
      }).length,
    [tickets],
  );
  ...
  const departmentData = useMemo(() => {
    ...
    tickets.forEach((t) => {
      const dept = (t.department || "Unassigned").toString();
      deptMap[dept] = (deptMap[dept] || 0) + 1;
    });

    const knownDepartments = departments.map((dept) => dept.name);
  ...
  const monthlyAreaSeries = useMemo(
    () => [{ name: "Tickets", data: monthlyData.map((d) => d.count) }],
    [monthlyData],
  );
  ...
  const departmentBarSeries = useMemo(
    () => [{ name: "Tickets", data: departmentData.map((d) => d.count) }],
    [departmentData],
  );
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/60 shadow-xs">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Support
              Agent &bull; IT Staff
            </span>
          </div>
          <p className="text-xs text-slate-500 font-medium leading-relaxed">
            System-wide Support Agent operational dashboard, department triage
            queue, and audit analytics.
          </p>
        </div>
