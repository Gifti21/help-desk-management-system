...
export default function AgentReportsPage() {
  ...
  const monthlyMetrics = useMemo(() => {
    ...
    const resolved = filteredMonthlyTickets.filter(
      (t) => t.status === "RESOLVED" || t.status === "CLOSED",
    ).length;
    const open = filteredMonthlyTickets.filter(
      (t) => t.status === "OPEN" || t.status === "IN_PROGRESS",
    ).length;
    const critical = filteredMonthlyTickets.filter(
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 bg-[#F4F7F6] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-bold bg-emerald-100 text-emerald-900 border border-emerald-300">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-700" /> Support
              Agent {user?.name || "Support Agent"}
            </span>
          </div>
          <p className="text-xs text-slate-600 mt-1">
            Review monthly resolution logs, audit ticket metrics, and export
            official CSV & formatted PDF activity reports.
          </p>
        </div>
        ...
        <ExportButtons
          tickets={
            filteredMonthlyTickets.length > 0 ? filteredMonthlyTickets : tickets
          }
      ...
      <Card className="bg-white border-slate-200 p-5 shadow-xs">
        ...
        {filteredMonthlyTickets.length === 0 ? (
          <p className="text-xs text-slate-500 py-8 text-center bg-slate-50 rounded-xl border border-slate-200">
            No ticket records found for {months[selectedMonth]}. All system
            tickets are available via the Export buttons above.
          </p>
        ) : (
                ...
                {filteredMonthlyTickets.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3 text-slate-600">
                      {t.creatorName || "Employee"}
                    </td>
                    <td className="p-3 text-slate-600">
                      {(t.category || t.department || "IT Support").toString()}
                    </td>
                    <td className="p-3">
