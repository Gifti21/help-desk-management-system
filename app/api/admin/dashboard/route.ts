...
export async function GET(request: NextRequest) {
    try {
        ...
        // Calculate resolved, pending, overdue for pie chart
        const resolvedCount = ticketsByStatus.find(s => s.status === 'RESOLVED')?._count.id || 0;
        const closedCount = ticketsByStatus.find(s => s.status === 'CLOSED')?._count.id || 0;
        const inProgressCount = ticketsByStatus.find(s => s.status === 'IN_PROGRESS')?._count.id || 0;
            ...
            data: {
                ...
                charts: {
                    ticketsByStatus: {
                        ...
                        pending: inProgressCount + openTickets,
