        ...
        return NextResponse.json({
            ...
            data: {
                overview: {
                    ...
                    totalDepartments,
                    totalCategories,
                    openTickets: ticketsByStatus.find(s => s.status === 'OPEN')?._count.id || 0,
                    inProgressTickets: ticketsByStatus.find(s => s.status === 'IN_PROGRESS')?._count.id || 0,
                    resolvedTickets: ticketsByStatus.find(s => s.status === 'RESOLVED')?._count.id || 0,
                    closedTickets: ticketsByStatus.find(s => s.status === 'CLOSED')?._count.id || 0,
                },
