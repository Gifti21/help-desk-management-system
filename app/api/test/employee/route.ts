...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stats = await Promise.all([
      ...
      prisma.ticket.count({
        where: { requesterId: employee.id, status: "OPEN" },
      }),
      prisma.ticket.count({
        where: { requesterId: employee.id, status: "IN_PROGRESS" },
      }),
      prisma.ticket.count({
        where: {
    ...
    console.log(
      `✅ Found ${categories.length} categories and ${departments.length} departments`,
    );
