...
export function TicketSummaryCard({ ticket }: TicketSummaryCardProps) {
  const deptName = typeof ticket.department === 'object' && ticket.department ? ticket.department.name : (ticket.department || 'IT Support');
  const catName = typeof ticket.category === 'object' && ticket.category ? ticket.category.name : (ticket.category || 'IT Support');
  const requesterName = ticket.creatorName || (ticket.requester ? getUserDisplayName(ticket.requester) : 'Employee');
  const assigneeName = ticket.assigneeName || (ticket.assignee ? getUserDisplayName(ticket.assignee) : 'Bontu (Support Agent)');
  ...
  return (
    <Card className="bg-white border-slate-200">
      <div className="space-y-4">
        ...
        <p className="text-slate-700" style={{ fontSize: BODY_SM.size }}>{ticket.description}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {details.map((detail) => (
            ...
            >
              <p className="text-slate-500 font-mono text-[10px] uppercase font-bold">{detail.label}</p>
              <p
