...
export function TicketCard({ ticket, showAssignee = true }: TicketCardProps) {
  const deptName = typeof ticket.department === 'object' && ticket.department ? ticket.department.name : (ticket.department || (typeof ticket.category === 'object' && ticket.category ? ticket.category.name : ticket.category) || 'IT Support');

  return (
    <Link href={`/dashboard/technician/queue`} className="block">
      <Card className="transition hover:-translate-y-0.5 hover:shadow-md bg-white border-slate-200">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1 space-y-2">
            ...
            >
              <span>{deptName}</span>
              <span>{ticket.creatorName || (ticket.requester ? getUserDisplayName(ticket.requester) : 'Employee')}</span>
          ...
          <div className="flex shrink-0 items-center gap-3 self-start">
            <div
              className="inline-flex items-center gap-1 rounded-full px-3 py-1 bg-emerald-50 text-emerald-900 border border-emerald-200"
            >
              <MessageSquare size={14} />
              <span style={{ fontSize: CAPTION_REGULAR.size }}>{ticket.comments?.length || ticket.commentCount || 0}</span>
            </div>
            {showAssignee ? (
              <span style={{ color: BODY_TEXT_GREY, fontSize: CAPTION_REGULAR.size }}>
                {ticket.assigneeName || (ticket.assignee ? getUserDisplayName(ticket.assignee) : 'Bontu')}
              </span>
            ) : null}
